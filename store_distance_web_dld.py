...
def calculate_distances_full(base_df, query_df):
    """计算查询点与门店距离，返回完整结果（独立店场景：店中店和独立店均需≥1km）"""
    results = []
...
# ========== 主应用 ==========
def main():
    st.title("📍 门店距离查询工具 - 独立店")
    st.markdown("计算查询点与门店之间的直线距离，判断是否符合开店距离要求")
    st.caption("规则：与店中店距离需 ≥ 1000米，与独立店距离需 ≥ 1000米")
    ...
    if 'result_df' in st.session_state and st.session_state.result_df is not None:
        ...
        with col2:
            pass_dzd = len(result_df[result_df['店中店≥1km'] == '✅ 是'])
            st.metric("店中店≥1km", f"{pass_dzd} / {len(result_df)}")
        with col3:
        ...
        with col1:
            ...
            st.download_button(
                label="📥 导出Excel结果",
                data=buffer,
                file_name=f"distance_result_dld_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            )
        with col2:
            ...
            st.download_button(
                label="📥 导出CSV结果",
                data=csv_buffer,
                file_name=f"distance_result_dld_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                mime="text/csv"
            )
